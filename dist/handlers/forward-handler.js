...
const PENDING_REQUESTS_TABLE_NAME = process.env.PENDING_REQUESTS_TABLE_NAME ?? '';
 ...
 */
async function findActiveSession(subdomain) {
 ...
 */
async function storePendingRequest(requestId, connectionId) {
...
async function pollForResponse(requestId) {
    const deadline = Date.now() + constants_1.REQUEST_TIMEOUT_MS;
...
const handler = async (event) => {
    ...
    if (!connectionId) {
        ...
        return {
            statusCode: 502,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify((0, errors_1.createErrorResponse)(errors_1.ErrorCode.TUNNEL_OFFLINE)),
        };
    }
    ...
    const body = event.body
        ? event.isBase64Encoded
            ...
            : event.body
        : null;
    const tunnelRequest = (0, serializer_1.serializeRequest)(event.httpMethod, fullPath, headers, body);
    ...
    const chunks = (0, chunker_1.chunkMessage)(serializedPayload, 'request');
    // Step 4: Send chunks via PostToConnection
    const managementClient = createManagementClient();
    try {
        for (const chunk of chunks) {
            await managementClient.send(new client_apigatewaymanagementapi_1.PostToConnectionCommand({
    ...
    if (!responsePayload) {
        ...
        return {
            statusCode: 504,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify((0, errors_1.createErrorResponse)(errors_1.ErrorCode.TUNNEL_TIMEOUT)),
        };
    }
    // Step 7: Deserialize and return the response
    const tunnelResponse = JSON.parse(responsePayload);
...
//# sourceMappingURL=forward-handler.js.map