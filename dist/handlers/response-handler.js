...
const handler = async (event) => {
    ...
    try {
        ...
        if (totalChunks === 1) {
            ...
            const assembled = await handleMultiChunk(messageId, chunkIndex, totalChunks, payload);
            if (!assembled) {
        ...
        const tunnelResponse = JSON.parse(completePayload);
        ...
        await dynamoClient.send(new client_dynamodb_1.UpdateItemCommand({
            ...
            UpdateExpression: 'SET #status = :status, #response = :response, updatedAt = :now',
 ...
 */
async function handleMultiChunk(messageId, chunkIndex, totalChunks, payload) {
    ...
    await dynamoClient.send(new client_dynamodb_1.UpdateItemCommand({
        ...
        UpdateExpression: 'SET #chunk = :payload, totalChunks = :total, updatedAt = :now, #ttl = :ttl ADD receivedCount :one',
        ExpressionAttributeNames: {
