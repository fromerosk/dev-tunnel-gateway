 ...
 */
function shouldBase64Encode(body, headers) {
    if (body === null)
        return false;
    ...
    if (headers) {
        ...
        if (contentType) {
            ...
            const isTextType = textTypes.some((t) => contentType.toLowerCase().includes(t));
...
function serializeRequest(method, path, headers, body) {
    if (body !== null) {
        ...
        if (size > constants_1.MAX_PAYLOAD_BYTES) {
            throw new Error(`Body size ${size} exceeds maximum payload size of ${constants_1.MAX_PAYLOAD_BYTES} bytes`);
        }
    }
 ...
 */
function deserializeRequest(msg) {
    let body = null;
    if (msg.body !== null) {
 ...
 * @throws Error if body exceeds MAX_PAYLOAD_BYTES
 */
function serializeResponse(statusCode, headers, body, requestId, startTime) {
    if (body !== null) {
        const size = Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body);
        if (size > constants_1.MAX_PAYLOAD_BYTES) {
            throw new Error(`Body size ${size} exceeds maximum payload size of ${constants_1.MAX_PAYLOAD_BYTES} bytes`);
 ...
 */
function deserializeResponse(msg) {
    let body = null;
    if (msg.body !== null) {
        if (msg.isBase64Encoded) {
            body = Buffer.from(msg.body, 'base64');
